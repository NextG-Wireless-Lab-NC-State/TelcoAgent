#!/usr/bin/env bash
# Wrapper around launch_thuml.sh that:
...
LOG="$LOG_DIR/thuml_supervised.log"
DONE_FILE="$LOG_DIR/thuml_supervised.done"
PID_FILE="$LOG_DIR/thuml_supervised.pid"
...
if [ -f "$PID_FILE" ] && kill -0 "$(cat "$PID_FILE")" 2>/dev/null; then
  echo "[$(date '+%F %T')] [THUML] supervisor already running pid=$(cat "$PID_FILE"); exiting" >> "$LOG"
  exit 0
fi

echo $$ > "$PID_FILE"
echo "[$(date '+%F %T')] [THUML] supervised start count=$COUNT pid=$$" >> "$LOG"

cd "$ROOT"
bash scripts/baseline_study/launch_thuml.sh "$COUNT" >> "$LOG" 2>&1
RC=$?

echo "[$(date '+%F %T')] [THUML] supervised exit rc=$RC" >> "$LOG"
if [ "$RC" -eq 0 ]; then
  touch "$DONE_FILE"
fi
rm -f "$PID_FILE"
